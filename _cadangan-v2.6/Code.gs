...
const KONFIG = {
  ...
  // Email yang berhak membuka panel admin langsung dari aplikasi.
  // Tambahkan email lain di sini kalau ada admin kedua.
...
function adalahAdmin(email) {
  ...
  return (KONFIG.ADMIN_EMAIL || []).some(function (x) { return String(x).toLowerCase() === e; });
}
...
function ujiPengaturan() {
  ...
  hasil.push('Email admin: ' + ((KONFIG.ADMIN_EMAIL || []).join(', ') || 'belum ada'));
  try {
    const sh = bukaSheet();
    bukaSheetAnggota();
